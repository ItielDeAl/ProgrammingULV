...
COLOR_EXITO = '#007BFF' # Azul para Simpson
COLOR_ERROR = '#DC3545' 

# --- LÓGICA MATEMÁTICA
def calcular_integral_simpson(funcion_str, a, b, n):
    x = sp.symbols('x')
    try:
    ...
    if n % 2 != 0:
        raise ValueError("La Regla de Simpson 1/3 requiere que el número de intervalos 'n' sea un entero par.")
    
    # Integral Simbólica (Valor Exacto)
    I_simbolica = sp.integrate(f_simbolica, (x, a, b))
    exacta = float(I_simbolica.evalf()) 

    # Cálculo Numérico (Regla de Simpson 1/3)
    h = (b - a) / n 
    ...
    suma_impares = 4 * np.sum(y_values[1:-1:2])
    suma_pares = 2 * np.sum(y_values[2:-1:2])
    
    integral_aprox = (h / 3) * (y_values[0] + suma_impares + suma_pares + y_values[-1])
    
    # Cálculo del Error
    error_abs_val = abs(exacta - integral_aprox)
    error_abs = float(error_abs_val)
...
class SimpsonApp:
    ...
    def crear_widgets(self):
        ...
        ttk.Label(self.main_frame, text="🔵 Regla de Simpson", font=("Arial", 16, "bold"), foreground=COLOR_EXITO).pack(pady=(0, 15), anchor='w')
        ...
        self.entrada_funcion.insert(0, "x**4")
        ...
        ttk.Label(frame_entrada, text="Intervalos 'n' (Par):").grid(row=4, column=0, padx=5, pady=5, sticky='w')
        ...
        ttk.Label(frame_resultados, text="Valor aproximado (Simpson):").grid(row=2, column=0, padx=5, pady=2, sticky='w')
    ...
    def calcular(self):
        ...
        try:
            ...
            exacta, aproximada, _, error_abs = calcular_integral_simpson(y_str, a, b, n)
    ...
    def graficar(self):
        ...
        try:
            y_str, a, b, n = self.obtener_parametros()
            _, _, f_numerica, _ = calcular_integral_simpson(y_str, a, b, n)
            ...
            ejes.scatter(x_simpson, f_numerica(x_simpson), color='red', s=15, zorder=5, label='Nodos de Simpson') 
